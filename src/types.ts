...
export interface TwitterCardAudit {
  present: boolean;
  card: string | null; // e.g. 'summary', 'summary_large_image'
  title: string | null;
  description: string | null;
  image: string | null;
  site: string | null;
...
export interface ImageAltAuditItem {
  ...
  hasDimensions: boolean; // width & height present to avoid CLS
  width?: string;
  height?: string;
  isLazy: boolean;
  format?: string;
...
export interface ImageAltAudit {
  ...
  imagesMissingDimensions: number; // CLS risk
...
export interface LighthouseVitalsAudit {
  ...
  coreWebVitals: {
    cls: {
      score: number; // Cumulative Layout Shift e.g. 0.04
    ...
    lcp: {
      valueMs: number | null; // Largest Contentful Paint
      formatted: string;
      rating: 'GOOD' | 'NEEDS_IMPROVEMENT' | 'POOR' | 'N/A';
    };
    inp: {
      valueMs: number | null; // Interaction to Next Paint
      formatted: string;
      rating: 'GOOD' | 'NEEDS_IMPROVEMENT' | 'POOR' | 'N/A';
    };
    fcp: {
      valueMs: number | null; // First Contentful Paint
      formatted: string;
      rating: 'GOOD' | 'NEEDS_IMPROVEMENT' | 'POOR' | 'N/A';
    };
    ttfb: {
      valueMs: number | null; // Time to First Byte
      formatted: string;
      rating: 'GOOD' | 'NEEDS_IMPROVEMENT' | 'POOR' | 'N/A';
    };
  };
...
export interface HumanIntentKeyword {
  ...
  inH1: boolean;
  inHeadings: boolean;
  intentSignal: string; // Explanation of why this intent was determined
}
...
export interface HumanIntentAnalysis {
  ...
  intentDistribution: {
    informational: number; // percentage e.g. 60
    commercial: number;
    transactional: number;
    navigational: number;
  };
  intentAlignmentScore: number; // 0 - 100 (checks if page type aligns with copy intent)
  ...
  informationGainInsights: {
    ...
    repetitiveFluffScore: number; // 0 (clean) to 100 (stuffed/slop)
    verdict: string;
  };
  recommendations: string[];
}
