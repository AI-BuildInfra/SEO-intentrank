...
export function extractNgrams(words: string[], n: number): Map<string, number> {
...
export function classifyIntent(phraseOrText: string): {
  ...
  for (const trigger of INFORMATIONAL_TRIGGERS) {
    if (new RegExp(`\\b${trigger}\\b`, 'i').test(text)) infoScore += 2;
  }
  for (const trigger of COMMERCIAL_TRIGGERS) {
    if (new RegExp(`\\b${trigger}\\b`, 'i').test(text)) commScore += 2.5;
  }
  for (const trigger of TRANSACTIONAL_TRIGGERS) {
    if (new RegExp(`\\b${trigger}\\b`, 'i').test(text)) transScore += 3;
  }
  for (const trigger of NAVIGATIONAL_TRIGGERS) {
    if (new RegExp(`\\b${trigger}\\b`, 'i').test(text)) navScore += 3;
  }

  const scores = [
    { intent: 'transactional' as SearchIntentType, score: transScore, trigger: 'High-conversion buying signals / pricing / signup triggers' },
    { intent: 'commercial' as SearchIntentType, score: commScore, trigger: 'Evaluation, review, or comparison modifiers' },
    { intent: 'navigational' as SearchIntentType, score: navScore, trigger: 'Brand navigation / portal / login indicators' },
    { intent: 'informational' as SearchIntentType, score: infoScore, trigger: 'Exploratory question or educational terminology' },
  ];
