...
export interface TechnicalSeoResult {
  ...
  contentMetrics: {
    ...
    textToHtmlRatio: number; // percentage e.g. 18.5%
...
export function auditHeadingHierarchy(doc: ParsedHtmlDocument): HeadingHierarchyAudit {
  ...
  let structureStatus: HeadingHierarchyAudit['structureStatus'] = 'optimal';
  if (h1s.length === 0) {
  ...
  if (skippedLevels.length > 0) {
    if (structureStatus === 'optimal') structureStatus = 'skipped_levels';
...
export function auditSchemaOrg(doc: ParsedHtmlDocument): SchemaOrgAudit {
  const schemas: any[] = [];
    ...
    try {
      ...
      const extractTypes = (obj: any) => {
        ...
        if (Array.isArray(obj['@graph'])) {
          for (const item of obj['@graph']) extractTypes(item);
        }
      };
      extractTypes(parsed);
...
export function parseRobotsTxt(content: string): { hasSitemapDirective: boolean; disallowCount: number } {
  ...
  for (const line of lines) {
    ...
    if (/^disallow\s*:/i.test(trimmed)) disallowCount++;
    if (/^sitemap\s*:/i.test(trimmed)) hasSitemapDirective = true;
  }

  return { hasSitemapDirective, disallowCount };
}

export function parseSitemapXml(content: string): { totalUrls: number; urls: string[] } {
  const urls: string[] = [];
...
export async function auditTechnicalSeo(input: TechnicalSeoInput): Promise<TechnicalSeoResult> {
  let html = input.html || '';
  let url = input.url;
  ...
  let robotsData: TechnicalSeoResult['robotsTxt'];
  let sitemapData: TechnicalSeoResult['sitemapXml'];
