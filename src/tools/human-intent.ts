...
export async function extractHumanIntentKeywords(input: HumanIntentInput): Promise<HumanIntentAnalysis> {
  let html = input.html || '';
  let text = input.text || '';
  const url = input.url;
  ...
  let doc: ParsedHtmlDocument | null = null;
  if (html) {
    doc = parseHtml(html);
    text = doc.plainText;
  } else if (!text) {
  ...
  const intentKeywords: HumanIntentKeyword[] = [];
  ...
  for (const [phrase, count] of candidatePhrases.entries()) {
    ...
    if (inTitle) score += 0.35;
    if (inH1) score += 0.30;
    if (inHeadings) score += 0.15;
  ...
  const synthesizedQueries: HumanSearchQuery[] = [];
  ...
  const minedQuestions: HumanIntentAnalysis['minedQuestions'] = rawQuestions.map((q) => ({
    question: q,
    isAnswered: true,
    source: 'content',
  }));
  ...
  for (const slop of aiSlopPhrases) {
    if (new RegExp(`\\b${slop}\\b`, 'i').test(text)) slopMatches++;
  }
