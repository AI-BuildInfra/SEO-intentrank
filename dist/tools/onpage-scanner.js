...
export function scanMetaTags(doc, baseUrl) {
    // Title
...
export function scanOpenGraph(doc) {
...
export function scanTwitterCard(doc) {
    ...
    if (!card)
        missingProperties.push('twitter:card');
    if (!title)
        missingProperties.push('twitter:title');
    if (!description)
        missingProperties.push('twitter:description');
    if (!image)
        missingProperties.push('twitter:image');
...
export function scanImageAltTags(doc) {
    const items = [];
    let imagesWithAlt = 0;
...
export async function scanOnPageSeo(input) {
    let html = input.html || '';
    let url = input.url;
    if (!html && url) {
        // Fetch live URL
    ...
    const recommendations = [];
    // Evaluate Meta Title
    if (meta.title.status === 'optimal')
        passed++;
    else if (meta.title.status === 'missing') {
        critical++;
        if (meta.title.recommendation)
            recommendations.push(meta.title.recommendation);
    }
    else {
        warnings++;
        if (meta.title.recommendation)
    ...
    if (meta.description.status === 'optimal')
        ...
        critical++;
        if (meta.description.recommendation)
            recommendations.push(meta.description.recommendation);
    }
    else {
        warnings++;
        if (meta.description.recommendation)
    ...
    // Evaluate Canonical
    if (meta.canonical.status === 'valid')
        passed++;
    else if (meta.canonical.status === 'missing') {
        warnings++;
        if (meta.canonical.recommendation)
            recommendations.push(meta.canonical.recommendation);
    }
    else {
        warnings++;
        if (meta.canonical.recommendation)
    ...
    // Evaluate Viewport
    if (meta.viewport.isMobileOptimized)
        passed++;
    else {
        critical++;
    ...
    // Evaluate Images
    if (images.imagesMissingAlt === 0 && images.imagesMissingDimensions === 0)
        passed++;
    else {
        if (images.imagesMissingAlt > 0)
            warnings++;
        if (images.imagesMissingDimensions > 0)
            warnings++;
