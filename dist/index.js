...
import { scanOnPageSeo } from './tools/onpage-scanner.js';
import { auditLighthouseVitals } from './tools/lighthouse-cls.js';
import { auditTechnicalSeo } from './tools/technical-seo.js';
    ...
    try {
        switch (name) {
            case 'scan_onpage_seo': {
                const result = await scanOnPageSeo(args || {});
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'audit_lighthouse_cls_vitals': {
                const result = await auditLighthouseVitals(args || {});
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'audit_technical_seo': {
                const result = await auditTechnicalSeo(args || {});
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'extract_human_intent_keywords': {
                const result = await extractHumanIntentKeywords(args || {});
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'comprehensive_seo_audit': {
                const result = await runComprehensiveSeoAudit(args || {});
                return {
                    content: [
                        {
                            type: 'text',
